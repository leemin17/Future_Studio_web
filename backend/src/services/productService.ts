...
export const getProducts = async (category?: ProductCategory): Promise<NewsItem[]> => {
  let query = requireSupabaseAdmin().from('products').select(productColumns).order('created_at', { ascending: false });
...
export const getProduct = async (id: number): Promise<NewsItem | null> => {
  const { data, error } = await requireSupabaseAdmin().from('products').select(productColumns).eq('id', id).maybeSingle();
  if (error) throw error;
...
export const deleteProduct = async (id: number): Promise<void> => {
  ...
  const { data: current, error: fetchError } = await client.from('products').select(productColumns).eq('id', id).maybeSingle();
