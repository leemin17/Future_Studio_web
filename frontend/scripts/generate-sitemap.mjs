...
const loadBrands = async () => {
  ...
  if (!supabaseUrl || !supabaseKey) return [];
  try {
    const response = await fetch(`${supabaseUrl}/rest/v1/brands?select=slug&is_visible=eq.true&order=display_order.asc`, { headers: { apikey: supabaseKey, Authorization: `Bearer ${supabaseKey}` } });
    if (!response.ok) throw new Error(`Supabase returned ${response.status}`);
    return await response.json();
  } catch (error) {
    console.warn(`Sitemap generated without brand URLs: ${error instanceof Error ? error.message : error}`);
    return [];
  }
};
...
const urls = [
  ...
  ...products.map((product) => `/projects/${slugify(product.title)}-${product.id}`),
];
